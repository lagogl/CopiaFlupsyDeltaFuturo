...
export class MemStorage implements IStorage {
  ...
  async completeScreeningOperation(id: number): Promise<ScreeningOperation | undefined> {
    ...
    // Per ora disabilitiamo questo controllo, gestiremo automaticamente la dismissione
    ...
    for (const sourceBasket of sourceBaskets) {
      if (!sourceBasket.dismissed) {
        ...
        if (cycle && cycle.state === 'active') {
          await this.updateCycle(cycle.id, {
            state: 'closed',
            endDate: currentDate
          });
    ...
    // 2. Creare nuovi cicli per le ceste di destinazione
    for (const destBasket of destinationBaskets) {
      ...
      const newCycle = await this.addCycle({
      ...
      if (newCycle) {
        ...
        await this.updateBasket(basket.id, {
        ...
        await this.updateScreeningDestinationBasket(destBasket.id, {
          cycleId: newCycle.id
        });
        ...
        await this.addOperation({
          ...
          averageWeight: destBasket.totalWeight && destBasket.animalCount ? (destBasket.totalWeight / destBasket.animalCount) * 1000 : null, // mg
