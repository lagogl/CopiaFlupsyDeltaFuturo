  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      for (const destBasket of destinationBasketsWithValidFlupsyId) {
        ...
        // CORREZIONE BUG: Semplificazione - registra solo nella tabella di relazione
        // senza creare cicli anticipati che causavano duplicazioni
        
        // Registra nella tabella di relazione temporanea con cycleId = 0 (placeholder)
        await tx.insert(selectionDestinationBaskets).values({
          selectionId: Number(id),
          basketId: destBasket.basketId,
          cycleId: 0, // Placeholder - il ciclo sarà creato durante completeSelection()
