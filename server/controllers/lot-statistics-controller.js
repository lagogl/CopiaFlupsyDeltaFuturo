/**
 * Controller per le statistiche dei lotti
 * Fornisce dati reali dal database senza utilizzare valori di fantasia
 */
...
/**
 * Recupera statistiche reali per i lotti dal database
 * @param {object} req - L'oggetto request
 * @param {object} res - L'oggetto response
 * @returns {Promise<void>} Risposta con statistiche reali dei lotti
 */
export async function getLotStatistics(req, res) {
  try {
    console.time('fetch-lot-statistics');
    
    // Calcola statistiche direttamente dal database in una singola query ottimizzata
    ...
    console.log('Statistiche lotti recuperate:', { 
      totalCount,
    ...
    return res.status(500).json({ 
      success: false,
      message: "Errore nel recupero delle statistiche dei lotti" 
    });
  }
}