...
export class ExternalSyncService {
  ...
  private mapSaleData(row: any, mapping: Record<string, string>): InsertExternalSaleSync {
    ...
    return {
      ...
      saleDate: mapped.saleDate ? new Date(mapped.saleDate).toISOString() : (row.sale_date ? new Date(row.sale_date).toISOString() : new Date().toISOString()),
      ...
      deliveryDate: mapped.deliveryDate ? new Date(mapped.deliveryDate).toISOString() : (row.delivery_date ? new Date(row.delivery_date).toISOString() : null),
