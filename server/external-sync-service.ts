...
export class ExternalSyncService {
  ...
  private mapSaleData(row: any, mapping: Record<string, string>): InsertExternalSaleSync {
    ...
    return {
      externalId: mapped.externalId || row.id,
      saleNumber: mapped.saleNumber || row.sale_number || '',
      saleDate: mapped.saleDate || row.sale_date,
      customerId: mapped.customerId || row.customer_id,
      customerName: mapped.customerName || row.customer_name || '',
      productCode: mapped.productCode || row.product_code,
      productName: mapped.productName || row.product_name || '',
      productCategory: mapped.productCategory || row.product_category,
      quantity: mapped.quantity || row.quantity || '0',
      unitOfMeasure: mapped.unitOfMeasure || row.unit_of_measure || 'kg',
      unitPrice: mapped.unitPrice || row.unit_price || '0',
      totalAmount: mapped.totalAmount || row.total_amount || '0',
      discountPercent: mapped.discountPercent || row.discount_percent || '0',
      discountAmount: mapped.discountAmount || row.discount_amount || '0',
      netAmount: mapped.netAmount || row.net_amount || '0',
      vatPercent: mapped.vatPercent || row.vat_percent || '22',
      vatAmount: mapped.vatAmount || row.vat_amount || '0',
      totalWithVat: mapped.totalWithVat || row.total_with_vat || '0',
      paymentMethod: mapped.paymentMethod || row.payment_method,
      deliveryDate: mapped.deliveryDate || row.delivery_date,
