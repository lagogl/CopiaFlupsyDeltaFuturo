...
export default function FlupsyComparison() {
  ...
  const renderCurrentBasket = (basket) => {
    ...
    return (
      <div 
        className={`basket-card p-2 rounded border-2 ${colorClass} h-32 flex flex-col justify-between`}
        onClick={() => {}}
      >
        ...
        {latestOperation && (
          <div className="mt-auto text-xs text-center">
            ...
            <div className="opacity-70 text-[10px]">{latestOperation.type}</div>
          </div>
        )}
      </div>
    );
  ...
  const renderFutureBasketBySize = (basket) => {
    ...
    return (
      <div 
        className={`basket-card p-2 rounded border-2 ${colorClass} h-32 flex flex-col justify-between ${!willReach ? 'opacity-40' : ''}`}
        onClick={() => {}}
      >
        <div className="flex justify-between items-start">
          <span className="font-bold text-xs">#{basket.physicalNumber}</span>
              ...
              {Array.from({ length: maxPosition }).map((_, idx) => {
                ...
                return (
                  <div key={`SX-${position}`} className="relative">
                    <div className="position-number absolute -top-5 left-1/2 transform -translate-x-1/2 text-xs text-gray-500">
                      {position}
                    </div>
  ...
  return (
    <div className="container mx-auto">
      ...
      <Card className="mb-6">
        <CardHeader className="pb-3">
          <CardTitle className="text-md">Legenda</CardTitle>
        </CardHeader>
        <CardContent>
