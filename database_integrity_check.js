/**
 * Script per il controllo dell'integrità del database e la pulizia dei record orfani
 * 
 * Questo script analizza le relazioni tra le tabelle e identifica record che non hanno
 * più riferimenti validi nelle tabelle correlate. Fornisce anche opzioni per ripulire
 * questi dati per mantenere l'integrità del database.
 */

const { db } = require('./server/db');
const { sql, eq, and, isNull, inArray } = require('drizzle-orm');
const {
  flupsys,
  baskets,
  cycles,
...
async function checkDatabaseIntegrity(fix = false) {
...
// 1. Operazioni senza cestelli validi
async function checkOrphanedOperations(fix) {
...
// 2. Cicli senza cestelli validi
async function checkOrphanedCycles(fix) {
...
// 3. Cestelli senza FLUPSY validi
async function checkOrphanedBaskets(fix) {
...
// 4. Posizioni cestelli senza cestelli o FLUPSY validi
async function checkOrphanedBasketPositions(fix) {
...
// 5. Record di vagliatura orfani (source baskets)
async function checkOrphanedScreeningSourceBaskets(fix) {
...
// 6. Record di vagliatura orfani (destination baskets)
async function checkOrphanedScreeningDestinationBaskets(fix) {
...
// 7. Record di storia vagliatura orfani
async function checkOrphanedScreeningBasketHistory(fix) {
...
async function checkIncompleteScreenings(fix) {
...
// 9. Vagliature orfane (senza cestelli associati)
async function checkOrphanedScreenings(fix) {
...
// 10. Riferimenti a taglie non esistenti
async function checkDanglingSizeReferences(fix) {
...
// 11. Riferimenti a lotti non esistenti
async function checkDanglingLotReferences(fix) {
...
// 12. Cestelli senza posizioni assegnate
async function checkBasketsWithoutPositions() {
...
async function main() {
  try {
    // Parametro 'true' per correggere gli errori, 'false' solo per report
    const fix = process.argv.includes('--fix');
    await checkDatabaseIntegrity(fix);
    console.log('\nControllo integrità database completato.');
    process.exit(0);
  } catch (error) {
    console.error('Errore durante il controllo integrità del database:', error);
    process.exit(1);
  }
}
