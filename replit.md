...
- **AI Report Generator**: Advanced report generation module with 10 comprehensive features:
  ...
  - **Multi-Step Pipeline**: In-memory data processing pipeline supporting complex multi-step operations (query → filter → aggregate → join → sort). Internal service only (no public endpoint for security). Validates pipeline structure, executes steps sequentially with intermediate result caching, supports both inner/left joins, and detailed execution logging. READ-ONLY enforced with SQL keyword validation.
